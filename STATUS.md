...
### Hotfix Déploiement Vercel — Script de Build Manquant (10 mars 2026)

**Contexte** : Le déploiement sur Vercel échouait avec l'erreur `bash: scripts/build-with-db.sh: No such file or directory`. Le fichier existait localement mais semblait absent ou inaccessible pour le processus de build distant.

**Correction** :
- ✅ **Renommage du script de build** : `scripts/build-with-db.sh` -> `scripts/vercel-build.sh` pour forcer la prise en compte par git (nouveau fichier).
- ✅ **Mise à jour configuration** : `package.json` et `vercel.json` pointent désormais vers le nouveau script.
- ✅ **Documentation** : `DEPLOYMENT.md` mis à jour.

**Fichiers modifiés** :
- `scripts/vercel-build.sh` (nouveau)
- `scripts/build-with-db.sh` (supprimé)
- `package.json`
- `vercel.json`
- `docs/DEPLOYMENT.md`

### Rebranding Complet — NeoSaaS vers Popurank (10 mars 2026)

**Contexte** : Le projet a été renommé de NeoSaaS vers Popurank pour correspondre à la nouvelle identité.

**Changements** :
- ✅ **Renommage Global** : Remplacement de "NeoSaaS" par "Popurank" dans tout le codebase (UI, Config, Emails, Documentation).
- ✅ **Configuration Email** : `no-reply@neosaas.tech` -> `no-reply@popurank.com`.
- ✅ **Meta-adonnées** : Titres de pages et descriptions mis à jour.
